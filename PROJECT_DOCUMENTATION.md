...
> 2026-09-18 后续边界修订：本文件保留历史实现说明；当前代码的统一 1100 AU 入射/移除球面、1 R_sun 匹配面和 1 AU 外层分箱上限见 [BOUNDARY_1100AU_REVISION.md](BOUNDARY_1100AU_REVISION.md)。
...
当前 C++ 主模拟不再把每条轨迹写成 `.dat` 文件，而是在内存中在线累积径向箱计数。基础网格为 `[0,1.1R_\odot)` 的 `1100` 个等宽 bin，bin 宽 `0.001R_\odot`（约 `695.7 km`）；外部网格从同样的 `0.001R_\odot` 宽度开始，每个 bin 按 `1.02` 的比例加宽，并设有 `10R_\odot` 的全局宽度上限。径向域不设截断，外部 bin 按轨道远日点动态扩展；输出行数随实际统计范围变化，最外层保留完整箱宽。每个已接受 RK45 区间由端点位置和速度构造三次 Hermite dense output；`1.1R_\odot` 外的束缚 Kepler 段则逐壳层加入解析的 $\Delta t$ 与 $v^2\Delta t$：

$$\sum \Delta t,\qquad \sum v^2 \Delta t$$

普通模式累积 captured 轨迹的 `bincount` 与每轨迹平方和，用于输出误差估计。束缚太阳外轨道无径向截断，完整往返的停留时间均计入统计，之后继续模拟至蒸发或其他终止条件；未捕获轨迹不输出 residence bincount。Capture Mode 为了快速扫描捕获率，会跳过 `bincount`、蒸发记录、snapshot 和反射谱数据累积，只保留轨迹总数、捕获数和捕获率误差。
...
- `bincount.txt`：captured 与 not_captured 的径向占据时间 $\sum \Delta t$、速度二阶矩 $\sum v^2\Delta t$，以及逐 bin 误差估计。`1.1R_\odot` 内使用 `0.001R_\odot` 等宽网格；外部宽度从 `0.001R_\odot` 开始按 `1.02` 的比例逐 bin 增长，箱宽最大为 `10R_\odot`，箱数按需增长，无径向截断。负能量粒子的太阳外 Kepler 段按解析壳层积分。
...
**实现内容**：轨迹、全局统计和 snapshot 使用 `std::vector<double>`，以 `1100` 个内区 bin 起步，按太阳外轨道的远日点扩展。MPI 汇总先同步最大长度并补零；jackknife 内存按 bin 优先排列，扩展时保留已有块统计。快照二进制版本为 `9`，读取前按文件实际长度校验箱数。

**效率影响**：不再通过有限径向域移除弱束缚轨道；极大远日点会增加直方图、jackknife 和输出文件的存储量。输出元数据中的 `radial_extent_Rsun` 记录实际网格范围，`radial_domain_max_AU` 为 `unbounded`，JSON 的 `outer_domain_removal_AU` 为 `null`。
...
**优化内容**：入射双曲轨道解析推进到 `1.1R_\odot`；捕获后负能量粒子从该边界外行时，解析推进到远日点并返回同一边界，同时把精确的 $\Delta t$ 与 $v^2\Delta t$ 加入动态混合网格：内区等宽、外区宽度按几何级数增长并封顶于 `10R_\odot`。任意可计算的束缚外轨道均经远日点返回，不按径向距离移除。

**物理影响**：所有束缚太阳外传播采用点质量 Kepler 解析解，省去弱束缚长周期轨道的大量数值步，完整保留远日点附近的停留时间。正能量粒子在 `1.1R_\odot` 外行时终止，不为无穷远轨道添加有限 bincount 尾部。
