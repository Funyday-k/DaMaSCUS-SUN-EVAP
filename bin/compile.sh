...
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P 2>/dev/null)" || SCRIPT_DIR="$PWD"
PROJECT_ROOT="${PROJECT_ROOT:-$(git -C "$SCRIPT_DIR" rev-parse --show-toplevel 2>/dev/null || echo /lustre/project/kennyng/damascus-earth/DaMaSCUS-NewEarth-EVAP)}"
