...
def get_base_element_from_sd_key(sd_key):
    """
    Convert SD key like 'Si29', 'Fe57', 'O17', 'H2' -> base element 'Si', 'Fe', 'O', 'H'.
    Exact element keys like 'Al', 'Na', 'P' are returned unchanged.
    """
    import re
...
def get_sd_spinful_isotopes_for_element(elem):
    """
    Return all SD-capable isotope entries in SD_SPIN_DB associated with the base element `elem`.

    Examples
    --------
    elem = 'Si'  -> may return ['Si29']
    elem = 'Fe'  -> may return ['Fe57']
    elem = 'O'   -> may return ['O17']
    elem = 'Al'  -> may return ['Al']
    """
    ...
    for sd_key, spin_data in SD_SPIN_DB.items():
        # Match exact key 'Al' or isotope-style key 'Si29'
        if sd_key == elem:
            pass
...
def sigma_nucleus_SD(DM_mass, elem, w_km_s, sigma_SD_p, sigma_SD_n=0.0, cross_section_type="constant"):
    ...
    base_elem = get_base_element_from_sd_key(elem)

    iso_ab = spin_data.get("isotope_abundance", spin_data.get("abundance", 1.0))
    S_p = spin_data.get("S_p", 0.0)
    S_n = spin_data.get("S_n", 0.0)

    A_eff = int(spin_data.get("isotope", round(ELEMENT_DB[base_elem]["A"])))
    m_A = A_eff * M_U_GEV
...
def diagnose_sd_element_contributions(
    ...
    if verbose:
        ...
        print("-" * 80)
        print(f"Top-{n_show} fraction sum = {frac_sum_inset-block-start: .6e}")
...
def diagnose_sd_element_contributions_detailed(
    ...
    if verbose:
        ...
        print(f"Top-{n_show} verified fraction       = {verified_fraction_inset-block-start:.6e}")
        print(f"Top-{n_show} placeholder fraction    = {placeholder_fraction_inset-block-start:.6e}")
...
def plot_combined_SI_verified_only_SD_electron_operator_grid(
    earth_data,
    DM_masses,
    sigma_SI_p=1e-40,
    sigma_SD_p=1e-40,
    ...
    Final-style combined operator grid:
        rows   = constant / v2_dependent / q2_dependent
        left   = absolute capture rates
        right  = ratio to constant for each channel
    """
    from concurrent.futures import ProcessPoolExecutor, as_completed
    ...
    with open(log_path, "w", encoding="utf-8") as f:
        ...
        f.write("  - rows = {constant, v2_dependent, q2_dependent}\n")
        f.write("  - left panels = absolute capture rates\n")
        f.write("  - right panels = ratio to constant for each channel\n")
        f.write("  - electron thermal motion disabled in baseline\n")
        f.write("  - SD mode = verified_only\n\n")
        f.write(f"sigma_SI_p         = {sigma_SI_p:.6e} cm^2\n")
        f.write(f"sigma_SD_p         = {sigma_SD_p:.6e} cm^2\n")
    ...
    # ------------------------------------------------------------
    axL = axes[0, 0]
    axL.loglog(masses_sorted, C_e_constant, color=color_e, linestyle=ls_e, linewidth=2.2, label="Electrons")
    axL.loglog(masses_sorted, C_SD_constant, color=color_sd, linestyle=ls_sd, linewidth=2.2, label="Nucleons-SD")
    axL.loglog(masses_sorted, C_SI_constant, color=color_si, linestyle=ls_si, linewidth=2.6, label="Nucleons-SI")
    axL.loglog(masses_sorted, C_geo, color=color_geo, linestyle=ls_geo, linewidth=1.5, alpha=0.7, label="Geometric")
    axL.set_ylabel(r"$C\ [{\rm s}^{-1}]$", fontsize=12)
    axL.set_title(r"constant, $T=0$", fontsize=13)
    axL.grid(True, which="both", alpha=0.3)
    axL.legend(fontsize=9, loc="best")
    axL.text(
        0.98, 0.97,
    ...
    axR = axes[0, 1]
    axR.semilogx(masses_sorted, np.ones_like(masses_sorted), color=color_e, linestyle=ls_e, linewidth=2.2, label="Electrons / constant")
    axR.semilogx(masses_sorted, np.ones_like(masses_sorted), color=color_sd, linestyle=ls_sd, linewidth=2.2, label="Nucleons-SD / constant")
    axR.semilogx(masses_sorted, np.ones_like(masses_sorted), color=color_si, linestyle=ls_si, linewidth=2.6, label="Nucleons-SI / constant")
    axR.axhline(1.0, color="black", linestyle=":", linewidth=1.1, alpha=0.8)
    axR.set_ylabel("ratio to constant", fontsize=12)
    axR.set_title(r"constant / constant", fontsize=13)
    axR.set_yscale("log")
    _set_ratio_panel_ylim(axR, [], top_row=True)
    axR.grid(True, which="both", alpha=0.3)
    axR.legend(fontsize=9, loc="best")
    ...
    # ------------------------------------------------------------
    axL = axes[1, 0]
    axL.loglog(masses_sorted, C_e_v2, color=color_e, linestyle=ls_e, linewidth=2.2, label="Electrons")
    axL.loglog(masses_sorted, C_SD_v2, color=color_sd, linestyle=ls_sd, linewidth=2.2, label="Nucleons-SD")
    axL.loglog(masses_sorted, C_SI_v2, color=color_si, linestyle=ls_si, linewidth=2.6, label="Nucleons-SI")
    axL.loglog(masses_sorted, C_geo, color=color_geo, linestyle=ls_geo, linewidth=1.5, alpha=0.7, label="Geometric")
    axL.set_ylabel(r"$C\ [{\rm s}^{-1}]$", fontsize=12)
    axL.set_title(r"$v^2$ dependent, $T=0$", fontsize=13)
    axL.grid(True, which="both", alpha=0.3)
    axL.legend(fontsize=9, loc="best")

    axR = axes[1, 1]
    axR.semilogx(masses_sorted, ratio_e_v2, color=color_e, linestyle=ls_e, linewidth=2.2, label="Electrons / constant")
    axR.semilogx(masses_sorted, ratio_SD_v2, color=color_sd, linestyle=ls_sd, linewidth=2.2, label="Nucleons-SD / constant")
    axR.semilogx(masses_sorted, ratio_SI_v2, color=color_si, linestyle=ls_si, linewidth=2.6, label="Nucleons-SI / constant")
    axR.axhline(1.0, color="black", linestyle=":", linewidth=1.1, alpha=0.8, label="constant reference")
    axR.set_ylabel("ratio to constant", fontsize=12)
    axR.set_title(r"$v^2$ / constant", fontsize=13)
    axR.set_yscale("log")
    _set_ratio_panel_ylim(axR, [ratio_e_v2, ratio_SD_v2, ratio_SI_v2], top_row=False)
    axR.grid(True, which="both", alpha=0.3)
    axR.legend(fontsize=9, loc="best")
    ...
    # ------------------------------------------------------------
    axL = axes[2, 0]
    axL.loglog(masses_sorted, C_e_q2, color=color_e, linestyle=ls_e, linewidth=2.2, label="Electrons")
    axL.loglog(masses_sorted, C_SD_q2, color=color_sd, linestyle=ls_sd, linewidth=2.2, label="Nucleons-SD")
    axL.loglog(masses_sorted, C_SI_q2, color=color_si, linestyle=ls_si, linewidth=2.6, label="Nucleons-SI")
    axL.loglog(masses_sorted, C_geo, color=color_geo, linestyle=ls_geo, linewidth=1.5, alpha=0.7, label="Geometric")
    axL.set_xlabel(r"$m_\chi\ [{\rm GeV}]$", fontsize=12)
    axL.set_ylabel(r"$C\ [{\rm s}^{-1}]$", fontsize=12)
    axL.set_title(r"$q^2$ dependent, $T=0$", fontsize=13)
    axL.grid(True, which="both", alpha=0.3)
    axL.legend(fontsize=9, loc="best")

    axR = axes[2, 1]
    axR.semilogx(masses_sorted, ratio_e_q2, color=color_e, linestyle=ls_e, linewidth=2.2, label="Electrons / constant")
    axR.semilogx(masses_sorted, ratio_SD_q2, color=color_sd, linestyle=ls_sd, linewidth=2.2, label="Nucleons-SD / constant")
    axR.semilogx(masses_sorted, ratio_SI_q2, color=color_si, linestyle=ls_si, linewidth=2.6, label="Nucleons-SI / constant")
    axR.axhline(1.0, color="black", linestyle=":", linewidth=1.1, alpha=0.8, label="constant reference")
    axR.set_xlabel(r"$m_\chi\ [{\rm GeV}]$", fontsize=12)
    axR.set_ylabel("ratio to constant", fontsize=12)
    axR.set_title(r"$q^2$ / constant", fontsize=13)
    axR.set_yscale("log")
    _set_ratio_panel_ylim(axR, [ratio_e_q2, ratio_SD_q2, ratio_SI_q2], top_row=False)
    axR.grid(True, which="both", alpha=0.3)
    axR.legend(fontsize=9, loc="best")
