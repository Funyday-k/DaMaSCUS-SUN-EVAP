...
def config_float(value: float) -> str:
    """Format a libconfig floating-point literal, including integral values."""
    text = f'{value:.12g}'
    return text if any(marker in text for marker in ('.', 'e', 'E')) else text + '.0'
    ...
    for i,(mass,exponent,outer) in enumerate(sorted(points)):
        ...
        for seed_index in range(repeats):
            for kind in (['thermal'] if phase=='thermal' else ['capture','transport']):
                ...
                folder=(destination/'results'/label/f'seed{seed_index}').resolve()
                replacements={'ID':f'"{label}_{kind}_{seed_index}"','run_mode':'"Capture"' if kind=='capture' else '"Parameter point"',
                              'sample_size':str(samples),'max_trajectories':'0','DM_mass':config_float(mass),
                              ...
                              'snapshot_enabled':str(kind=='transport').lower()}
                ...
                text+='snapshot_interval = 60;\ntrajectory_summary_enabled = false;\ntrajectory_events_enabled = false;\ntrajectory_trace_rate = 0.0;\n'
                if kind=='thermal': text+='maximum_number_of_scatterings = 10000;\n'
                cfg=destination/'configs'/label/f'seed{seed_index}'/f'{kind}.cfg'
                cfg.parent.mkdir(parents=True,exist_ok=True); cfg.write_text(text)
                manifest.append({'config':str(cfg.resolve()),'kind':kind,'mass_GeV':mass,'sigma_cm2':10.**exponent,'R_remove_rsun':outer,'samples':samples,'seed':seed,
                                 'log':str((destination/'logs'/f'{label}_{kind}_seed{seed_index}.out').resolve()),
                                 'output_dir':None if kind=='capture' else str(folder)})
