...
def build_parser():
    parser = argparse.ArgumentParser(description="Run electron-only all-cross-sections workflow.")
    add_mass_args(parser)
    add_earth_file_arg(parser)
    add_common_integration_args(parser, include_sigmas=("electron",))
    add_output_root_arg(parser, os.path.join(BASE_DIR, "outputs", "electron_all_cross_sections"))
    return parser
...
def main():
    ...
    print_header(
        "Electron-only all-cross-sections runner",
    ...
    results = call_supported(
        pt.run_electron_only_all_cross_sections,
