#!/usr/bin/run_sd_operator_target_decomposition.python3
import os
import sys
import argparse
import numpy as np
...
def parse_masses(args):
    ...
    Priority:
    1. --masses "1,10,100"
    2. otherwise use logspace from m_min to m_max with n_masses points
    """
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Run verified-only SD operator target decomposition at T=0."
    )
    ...
    parser.add_argument(
        ...
        help='Explicit comma-separated DM masses, e.g. "1,10,100". '
             "If omitted, logspace grid is used."
    )
    parser.add_argument(
        "--m-min",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Thermal speed grid size. For T=0 this is mostly interface consistency."
    )
    parser.add_argument(
        "--n-t-mu",
        type=int,
        default=4,
        help="Thermal angular grid size. For T=0 this is mostly interface consistency."
    )
    parser.add_argument(
        "--n-scatter-mu",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Radial shell stride. 1 means use every shell."
    )
    parser.add_argument(
        "--u-grid-mode",
        type=str,
    ...
    parser.add_argument(
        "--output-root",
        type=str,
        default=os.path.join(BASE_DIR, "outputs", "sd_operator_target_decomposition"),
...
def main():
    ...
    print("=" * 100)
    print("Verified-only SD operator target-decomposition runner")
    print("=" * 100)
    ...
    print(f"shell_step     = {args.shell_step}")
    print(f"u_grid_mode    = {args.u_grid_mode}")
    ...
    results = pt.plot_verified_only_sd_operator_target_decomposition(
