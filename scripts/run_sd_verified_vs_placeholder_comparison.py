...
def build_parser():
    parser = argparse.ArgumentParser(description="Run SD verified-only vs include-placeholders comparison.")
    ...
    add_output_root_arg(parser, os.path.join(BASE_DIR, "outputs", "sd_verified_vs_placeholder"))
    return parser
...
def main():
    ...
    print_header(
        "SD verified-only vs include-placeholders runner",
    ...
    results = call_supported(
        pt.plot_sd_verified_vs_include_placeholders_comparison,
        filepath=args.earth_file,
        earth_data=earth_verified,
