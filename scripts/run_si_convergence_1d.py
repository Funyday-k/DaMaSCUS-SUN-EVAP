...
def build_parser():
    parser = argparse.ArgumentParser(description="Run 1D SI convergence scan.")
    add_mass_args(parser, n_masses=4)
    add_earth_file_arg(parser)
    add_common_integration_args(parser, include_sigmas=("si",))
    add_output_root_arg(parser, os.path.join(BASE_DIR, "outputs", "si_convergence_1d"))
    return parser
