...
def build_parser():
    parser = argparse.ArgumentParser(description="Run SI convergence test at selected masses.")
    add_mass_args(parser, n_masses=4)
    add_earth_file_arg(parser)
    add_common_integration_args(parser, include_sigmas=("si",))
    add_output_root_arg(parser, os.path.join(BASE_DIR, "outputs", "si_convergence_selected_masses"))
    return parser
...
def main():
    ...
    print_header(
        "SI convergence selected-masses runner",
    ...
    results = call_supported(
        pt.test_convergence_at_selected_masses,
