...
# ------------------------------------------------------------
# Path setup
# scripts/ 位於專案根目錄之下，因此要把專案根目錄加到 sys.path
# ------------------------------------------------------------
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Run verified-only refined SD baseline."
    )
    ...
    # Physics / integration options
    parser.add_argument(
        "--sigma-sd-p",
        type=float,
        default=1e-40,
        help="SD proton reference cross section in cm^2."
    )
    parser.add_argument(
        "--u-max",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of worker processes. Default: os.cpu_count()-1 behavior in library."
    )
    ...
    parser.add_argument(
        "--output-root",
        type=str,
        default=os.path.join(BASE_DIR, "outputs", "verified_sd_baseline"),
...
def main():
    ...
    print("=" * 80)
    print("Verified-only refined SD baseline runner")
    print("=" * 80)
    ...
    earth_data = pt.load_earth_composition(sd_mode="verified_only")

    results = pt.plot_verified_only_refined_sd_baseline(
