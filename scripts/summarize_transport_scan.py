...
def comparison_signature(record: dict) -> str:
    ...
    settings={key:meta[key] for key in ['source_sha256','solar_model','halo_model','halo_density_GeV_cm3','R_inj_rsun','R_match_rsun']}
...
def summarize(files: list[Path], destination: Path) -> None:
    ...
    for file in files:
        ...
        for meta,workflow in [(m,'complete_captured_transport'),(c,'fixed_injection_capture')]:
            if (meta.get('production_accepted') is not True or meta.get('schema_version')!=6
