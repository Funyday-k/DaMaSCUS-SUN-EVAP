...
TEST(TestParameterScan, TransportControlsAreTypedAndNeverSilentlyIgnored)
{
    ...
    for(const auto& invalid : {"outer_removal_radius_rsun = \"3000\";", "outer_removal_radius_rsun = 0.005;", "outer_boundary_radius_au = 1100;", "production_mode = 1;", "thermal_validation_mode = \"false\";"}) {
        { std::ofstream out(path); out << text << "\n" << invalid << "\n"; }
